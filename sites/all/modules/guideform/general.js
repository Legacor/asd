...
function setCharAt(str, index, chr) { /// IMPORTANTE!! MOVER A GENERAL!!!!!!!!!!
    if (index > str.length - 1)
        return str;
    return str.substr(0, index) + chr + str.substr(index + 1);
}
function deleteFromString(x, index, n) {
    return x.substr(0, index) + x.substr(index + n);
}


function updateGuideField($fieldContainer, id, idValue, fieldType, color) {
    var fieldValue = $fieldContainer.val().toString();
    var index = fieldValue.indexOf(id);
    if (index > -1) { //verifica si ya existe un valor en el campo con ese id
        var newFieldValue = "";
        if (parseInt(idValue) === 0) {
            if (fieldType === "items") {
                var n = id.toString().length + 1;
                console.log("n: " + n);
            } else {
                var n = getN(fieldType);
            }
            newFieldValue = deleteFromString(fieldValue, index, n);
        } else {
            index = index + id.length + 1;
            newFieldValue = setCharAt(fieldValue, index, idValue); //se asigna el valor de la nueva cantidad al id existente
        }        
        $fieldContainer.val(newFieldValue);
    } else if (id!==null && id!==undefined)  { //si no existe, simplemente se agrega id con el num
        if (fieldType === "mastery" || fieldType === "skill")
            $fieldContainer.val($fieldContainer.val() + id + "=" + idValue + ",");
        else if (fieldType === "rune")
            $fieldContainer.val($fieldContainer.val() + id + "=" + idValue + "-" + color + ",");
        else if (fieldType === "items")
            $fieldContainer.val($fieldContainer.val() + id + ",");
    }
}
